...
import { startWatch, stopWatch } from "../git/watcher";
...
export function registerRepositoryHandlers(): void {
	...
	ipcMain.handle("repo:watch", async (event, repoPath: string) => {
		const win = BrowserWindow.fromWebContents(event.sender);
		if (!win) return;
		console.log(`Starting watch for ${repoPath} in window: ${win.id}`);
		await startWatch(win.id, repoPath, event.sender);
		win.once("closed", () => stopWatch(win.id));
	});
