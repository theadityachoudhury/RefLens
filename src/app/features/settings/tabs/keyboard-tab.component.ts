...
import { eventToCombo, displayCombo } from '../../../core/services/shortcut.service';
...
const FIXED_ROWS: FixedRow[] = [
  ...
  { label: 'Run command',        binding: 'Enter',           hint: 'Execute the run command in the worktree preview' },
  { label: 'New Window',         binding: 'Mod+N',           hint: 'Open a new application window (system menu)' },
];
...
@Component({
  ...
  template: `
    <div class="settings-section">
      <h2 class="settings-section__title">Configurable Shortcuts</h2>
          ...
          <div class="shortcut-row" [class.shortcut-row--editing]="editingAction() === row.action">
            ...
            <div class="shortcut-row__binding">
              @if (editingAction() === row.action) {
                ...
                <kbd class="shortcut-key">{{ display(s.snapshot.keyboardShortcuts[row.action]) }}</kbd>
    ...
    <div class="settings-section">
      ...
      <div class="shortcuts-table shortcuts-table--fixed">
        @for (row of fixedRows; track row.label) {
          <div class="shortcut-row">
            ...
            <div class="shortcut-row__binding">
              <kbd class="shortcut-key shortcut-key--fixed">{{ row.binding }}</kbd>
            </div>
          </div>
        }
      </div>
  ...
  styles: [`
    ...
    .shortcut-row {
      ...
      &--editing { background: var(--accent-subtle); padding: 0.75rem 0.75rem; border-radius: 6px; margin: 0 -0.75rem; border-bottom: none; }
...
export class SettingsKeyboardTabComponent {
  ...
  protected readonly recordingDisplay = () => {
    const c = this.pendingCombo();
    return c ? displayCombo(c) : '';
  };

  display(combo: string): string {
  ...
  onRecord(e: KeyboardEvent): void {
    ...
    this.pendingCombo.set(eventToCombo(e));
  }
